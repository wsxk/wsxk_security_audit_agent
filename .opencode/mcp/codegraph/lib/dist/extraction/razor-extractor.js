...
class RazorExtractor {
    ...
    createComponentNode() {
        ...
        const node = {
            id: (0, tree_sitter_helpers_1.generateNodeId)(this.filePath, 'component', componentName, 1),
            kind: 'component',
            name: componentName,
    ...
    typeNames(expr) {
        ...
        for (const raw of expr.split(/[<>,\s]+/)) {
            ...
            if (/^[A-Z][A-Za-z0-9_]*$/.test(seg))
                out.push(seg);
        }
        return out;
    }
    pushRef(componentId, name, line, column) {
    ...
    extractComponentTags(componentId) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            while ((m = tagRe.exec(line)) !== null) {
                const name = m[1];
                if (BLAZOR_BUILTIN_COMPONENTS.has(name))
                    continue;
                ...
                for (const t of (m[2] || '').matchAll(/\bT[A-Za-z]*\s*=\s*"([A-Za-z_][\w.]*)"/g)) {
                    ...
                    if (/^[A-Z]/.test(seg))
                        this.pushRef(componentId, seg, i + 1, 0);
                }
            }
        }
    }
     ...
     */
    processCodeBlocks(componentId) {
        if (!(0, grammars_1.isLanguageSupported)('csharp'))
            return;
        for (const block of this.extractCodeBlocks()) {
            ...
            let result;
            try {
                result = new tree_sitter_1.TreeSitterExtractor(this.filePath, `class __RazorCode__ {\n${block.content}\n}`, 'csharp').extract();
            }
            catch {
...
exports.RazorExtractor = RazorExtractor;
//# sourceMappingURL=razor-extractor.js.map