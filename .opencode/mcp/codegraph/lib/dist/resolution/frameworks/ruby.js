...
exports.railsResolver = {
    ...
    extract(filePath, content) {
        ...
        while ((match = resRegex.exec(safe)) !== null) {
            ...
            const symList = (s) => new Set(s.split(',').map((x) => x.trim().replace(/^:/, '')));
            if (only) {
                const s = symList(only[1]);
                actions = actions.filter((a) => s.has(a));
            }
            else if (except) {
                const s = symList(except[1]);
                actions = actions.filter((a) => !s.has(a));
            }
...
function resolveControllerAction(ctrlPath, action, context) {
